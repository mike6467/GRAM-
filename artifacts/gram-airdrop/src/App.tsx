...
function PreviewModal({ close }: { close: () => void }) {
  ...
  return <div className="modal-backdrop fixed inset-0 z-50 flex items-center justify-center p-4" role="dialog" aria-modal="true" aria-labelledby="preview-title">
    <div className="relative w-full max-w-[480px] overflow-hidden rounded-[26px] border border-[#9ec9c4] bg-[#f3faf8] shadow-2xl">
      <button type="button" onClick={close} data-testid="button-close-preview" aria-label="Close eligibility preview" className="absolute right-5 top-5 rounded-full p-2 hover:bg-[#dfefec] focus-ring"><X size={18} /></button>
      <div className="grid-paper border-b border-[#c8dfdc] px-7 pb-6 pt-8"><div className="eyebrow">Campaign preview / 01</div><h2 id="preview-title" className="display mt-4 max-w-[350px] text-3xl font-bold leading-[.98]">Find your place in the network.</h2><p className="mt-3 max-w-[380px] text-sm leading-6 text-[#5e747a]">A quick, non-binding walkthrough of how eligibility could be checked when official criteria are published.</p></div>
      {step === 1 ? <form onSubmit={submit} className="space-y-5 p-7">
        <label className="block"><span className="mono text-[10px] uppercase tracking-[.14em] text-[#557078]">Telegram handle <span className="text-[#9aabac]">(optional to campaign)</span></span><div className="mt-2 flex items-center rounded-xl border border-[#bfd9d5] bg-white px-3 focus-within:border-[#188d82]"><span className="text-[#819395]">@</span><input value={handle} onChange={e => setHandle(e.target.value.replace('@',''))} data-testid="input-telegram-handle" className="w-full bg-transparent px-2 py-3 text-sm outline-none" placeholder="yourhandle" /></div></label>
        <label className="flex cursor-pointer items-start gap-3 text-sm leading-5 text-[#526970]"><input type="checkbox" checked={safe} onChange={e => setSafe(e.target.checked)} data-testid="input-safety-confirmation" className="mt-1 h-4 w-4 accent-[#188d82]" />I understand this is an eligibility preview, not a guaranteed allocation.</label>
        <button type="submit" disabled={!handle.trim() || !safe} data-testid="button-submit-preview" className="btn-primary w-full disabled:cursor-not-allowed disabled:opacity-45">Show my preview <ArrowRight size={16} /></button>
        <p className="flex items-start gap-2 text-[11px] leading-4 text-[#7b8f92]"><ShieldCheck size={14} className="mt-0.5 shrink-0 text-[#188d82]" />Never share a seed phrase or private key. Verify every official link before connecting anything.</p>
      </form> : <div className="p-7"><div className="flex h-12 w-12 items-center justify-center rounded-full bg-[#d8f0e9] text-[#188d82]"><Check size={24} /></div><h3 className="display mt-5 text-2xl font-bold">Preview assembled.</h3><p className="mt-3 text-sm leading-6 text-[#5e747a]">You’re on the early signal list for <span className="font-bold text-[#188d82]">@{handle}</span>. Keep an eye on verified Gram and TON channels for any real announcement.</p><div className="mt-6 rounded-xl bg-[#e5f3ef] p-4 mono text-[10px] uppercase leading-5 tracking-[.08em] text-[#36736d]">STATUS / NOT ELIGIBLE YET<br /><span className="text-[#188d82]">SIGNAL / RECORDED FOR PREVIEW</span></div><button type="button" onClick={close} data-testid="button-finish-preview" className="btn-secondary mt-6 w-full">Return to the network</button></div>}
    </div>
  </div>;
}
