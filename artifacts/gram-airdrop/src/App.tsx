...
import { ArrowDownRight, ArrowRight, Check, ChevronDown, CircleHelp, ExternalLink, Info, LockKeyhole, Menu, ShieldCheck, Sparkles, WalletCards, X, Zap } from 'lucide-react';

type FAQItem = { question: string; answer: string };
type WalletOption = { name: string; mark: string; detail: string; tone: string };

const walletOptions: WalletOption[] = [
  { name: 'Tonkeeper', mark: 'TK', detail: 'TON mobile wallet', tone: 'teal' },
  { name: 'Telegram Wallet', mark: 'TW', detail: 'Built into Telegram', tone: 'blue' },
  { name: 'MyTonWallet', mark: 'MW', detail: 'Web3 TON wallet', tone: 'violet' },
  { name: 'TON HUB', mark: 'TH', detail: 'Community wallet', tone: 'yellow' },
  { name: 'OpenMask', mark: 'OM', detail: 'Browser extension', tone: 'orange' },
];
...
function Header({ onPreview }: { onPreview: () => void }) {
  ...
  return <header className="absolute inset-x-0 top-0 z-20">
    ...
    {open && <div className="mx-4 rounded-2xl border border-[#c3d9d8] bg-[#f8fcfb]/95 p-3 shadow-xl backdrop-blur md:hidden">
      {links.map(([label, id]) => <button type="button" key={id} onClick={() => go(id)} data-testid={`link-mobile-${id}`} className="flex w-full items-center justify-between rounded-xl px-4 py-3 text-left text-sm font-bold hover:bg-[#e4f1ef]">{label}<ArrowRight size={16} className="text-[#188d82]" /></button>)}
...
function PreviewModal({ close }: { close: () => void }) {
  ...
  return <div className="modal-backdrop fixed inset-0 z-50 flex items-center justify-center p-4" role="dialog" aria-modal="true" aria-labelledby="preview-title">
    <div className="relative w-full max-w-[520px] overflow-hidden rounded-[26px] border border-[#2a5961] bg-[#102630] text-[#e7f8f4] shadow-[0_30px_100px_rgba(0,0,0,.45)]">
      ...
      <div className="grid-paper border-b border-[#24505a] px-7 pb-6 pt-8"><div className="eyebrow">Campaign preview / {String(step).padStart(2, '0')}</div><h2 id="preview-title" className="display mt-4 max-w-[410px] text-3xl font-bold leading-[.98]">{step === 1 ? 'Find your place in the network.' : step === 2 ? 'Your signal is confirmed.' : step === 3 ? 'Choose your connection route.' : 'Approve the connection safely.'}</h2><p className="mt-3 max-w-[420px] text-sm leading-6 text-[#9bb9bb]">{step === 1 ? 'Enter your Telegram handle for a quick eligibility walkthrough. No wallet connection is needed to check.' : step === 2 ? 'Your preview result is ready. This is campaign UI only and does not guarantee a live allocation.' : step === 3 ? 'Choose the wallet interface you already use. You will approve any real connection inside that wallet, never on this page.' : `A connection request is ready for ${selectedWallet?.name ?? 'your wallet'}.`}</p></div>
      {step === 1 ? <form onSubmit={submit} className="space-y-5 p-7">
       ...
       </form> : step === 2 ? <div className="p-7"><div className="eligibility-burst flex h-14 w-14 items-center justify-center rounded-full bg-[#45bbae] text-[#08232b]"><Check size={28} /></div><div className="mono mt-6 text-[10px] uppercase tracking-[.16em] text-[#66cabc]">Eligibility result / @{handle}</div><h3 className="display mt-3 text-5xl font-bold tracking-[-.06em] text-[#f4fffc]">1,290 <span className="text-[#45bbae]">GRAM</span></h3><p className="mt-4 text-sm leading-6 text-[#a9c5c5]">Your campaign preview says you’re eligible for 1,290 GRAM TOKENS. Connect a wallet to continue to the safe claim preview.</p><div className="mt-6 flex items-start gap-3 rounded-xl border border-[#2a5961] bg-[#0b1c25] p-4 text-[11px] leading-5 text-[#86a9aa]"><Info size={16} className="mt-0.5 shrink-0 text-[#e3b743]" />Eligibility shown here is a product preview. Verify any real airdrop announcement through official Gram and TON channels.</div><button type="button" onClick={() => setStep(3)} data-testid="button-connect-wallet" className="btn-primary mt-6 w-full">Connect wallet to claim <WalletCards size={17} /></button><button type="button" onClick={() => setStep(1)} data-testid="button-back-to-handle" className="btn-secondary mt-3 w-full !border-[#2a5961] !bg-transparent !text-[#b1cccb]">Edit Telegram name</button></div>
       : step === 3 ? <div className="p-7"><div className="wallet-grid">{walletOptions.map(wallet => <button type="button" key={wallet.name} onClick={() => { setSelectedWallet(wallet); setStep(4); }} data-testid={`button-wallet-${wallet.name.toLowerCase().replaceAll(' ', '-')}`} className="wallet-option"><span className={`wallet-mark wallet-${wallet.tone}`}>{wallet.mark}</span><span className="min-w-0 text-left"><strong>{wallet.name}</strong><small>{wallet.detail}</small></span><ArrowRight size={16} className="ml-auto shrink-0 text-[#6f999b]" /></button>)}</div><div className="mt-6 flex items-start gap-3 rounded-xl border border-[#2a5961] bg-[#0b1c25] p-4 text-[11px] leading-5 text-[#86a9aa]"><LockKeyhole size={16} className="mt-0.5 shrink-0 text-[#45bbae]" />A real wallet connection uses your wallet’s own approval screen. Never type recovery words into a website.</div><button type="button" onClick={() => setStep(2)} data-testid="button-back-to-result" className="btn-secondary mt-5 w-full !border-[#2a5961] !bg-transparent !text-[#b1cccb]">Back to eligibility result</button></div>
       : <div className="p-7"><div className="flex h-14 w-14 items-center justify-center rounded-2xl bg-[#203f4a] text-[#45bbae]"><WalletCards size={27} /></div><div className="mono mt-6 text-[10px] uppercase tracking-[.16em] text-[#66cabc]">Connection route selected</div><h3 className="display mt-3 text-3xl font-bold">{selectedWallet?.name}</h3><p className="mt-3 text-sm leading-6 text-[#a9c5c5]">Open the official {selectedWallet?.name} app or extension and approve the connection request there. This page does not collect credentials.</p><div className="mt-6 rounded-xl border border-[#2a5961] bg-[#0b1c25] p-4"><div className="flex items-center gap-2 text-sm font-bold text-[#e7f8f4]"><LockKeyhole size={15} className="text-[#45bbae]" />Non-custodial connection</div><p className="mt-2 text-[11px] leading-5 text-[#86a9aa]">No seed phrase. No private key. No payment. If any page asks for those, close it immediately.</p></div><button type="button" onClick={() => setStep(3)} data-testid="button-change-wallet" className="btn-primary mt-6 w-full">Choose a different wallet <WalletCards size={17} /></button><button type="button" onClick={close} data-testid="button-close-safe-flow" className="btn-secondary mt-3 w-full !border-[#2a5961] !bg-transparent !text-[#b1cccb]">Close safe preview</button></div>}
    </div>
  </div>;
}
      ...
      <section className="grid-paper relative min-h-[760px] overflow-hidden border-b border-[#c9dfdc] pt-[120px] md:min-h-[840px] md:pt-[164px]" aria-labelledby="hero-title">
        <div className="container-wide grid items-center gap-8 pb-20 lg:grid-cols-[1.02fr_.98fr] lg:pb-28">
          <div className="relative z-10">
            ...
            <h1 id="hero-title" className="hero-title display reveal delay-1 mt-6 max-w-[720px] text-[clamp(4.7rem,10.2vw,9.4rem)] font-bold leading-[.84] tracking-[-.08em] text-[#1b3441]">Move at<br /><span className="text-[#188d82]">conversation</span><br />speed.</h1>
